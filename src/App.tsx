...
import { FOODS, FOOD_ICONS, ROLES, ROLE_ICONS, type Food, type Role } from "./domain/catalog";
import { changePlayerStatus, changeRound, createGame, cycleRoleMark, getCandidateRoles, getPublicRoleCount, replacePlayer, toggleFoodEvidence } from "./domain/game";
import type { DeathCause, FoodEvidence, Game, PlayerRecord, PlayerStatus } from "./domain/model";
...
function PlayerDrawer({ game, player, onChange, onClose }: { game: Game; player: PlayerRecord; onChange: (player: PlayerRecord) => void; onClose: () => void }) {
  const candidates = getCandidateRoles(game, player);
  const patch = (values: Partial<PlayerRecord>) => onChange({ ...player, ...values });
  const toggleFood = (food: Food, key: keyof FoodEvidence) => onChange(toggleFoodEvidence(player, food, key));
  const cycleRole = (role: Role) => onChange(cycleRoleMark(player, role));
  ...
  return <div className="drawer-backdrop" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
    <aside className="drawer" aria-label={`${player.name}的推理记录`}>
      ...
      <section className="candidate-strip">
        <div><small>系统候选</small><strong>{candidates.length} 个身份</strong></div>
        <div className="candidate-icons">{candidates.length ? candidates.map((role) => <span key={role} title={role}>{ROLE_ICONS[role]}</span>) : <em>线索存在矛盾</em>}</div>
      </section>

      <section><div className="section-title"><h2>食物线索</h2><span>“持有”不会参与自动排除</span></div>
        <div className="food-grid">{FOODS.map((food) => {
          ...
          return <article className="food-card" key={food}><div className="food-name"><i>{FOOD_ICONS[food]}</i><b>{food}</b></div><div className="evidence-row">
            <button className={evidence.held ? "on held" : ""} onClick={() => toggleFood(food, "held")}>持有</button>
            <button className={evidence.ate ? "on ate" : ""} onClick={() => toggleFood(food, "ate")}>吃过</button>
            <button className={evidence.cannot ? "on cannot" : ""} onClick={() => toggleFood(food, "cannot")}>不吃</button>
          </div></article>;
        })}</div>
      </section>

      <section><div className="section-title"><h2>身份判断</h2><span>点击循环：可能 → 怀疑 → 排除</span></div>
        <div className="role-pills">{ROLES.map((role) => <button key={role} data-mark={player.roleTags[role]} className={candidates.includes(role) ? "candidate" : ""} onClick={() => cycleRole(role)}><span>{ROLE_ICONS[role]}</span>{role}</button>)}</div>
      </section>
    ...
    <section className="table-area">
      ...
      <div className="round-table">
        ...
        {game.players.map((player, index) => {
          const angle = (index / game.playerCount) * Math.PI * 2 - Math.PI / 2;
          const x = 50 + Math.cos(angle) * 46;
          const y = 50 + Math.sin(angle) * 43;
          const isSelf = player.seat === game.selfSeat;
          const clues = FOODS.filter((food) => player.foodTags[food].ate || player.foodTags[food].cannot).slice(0, 3);
          return <button key={player.seat} className={`player-token ${isSelf ? "self" : ""} ${player.status}`} style={{ left: `${x}%`, top: `${y}%` }} onClick={() => !isSelf && setSelectedSeat(player.seat)} disabled={isSelf}>
            ...
            {clues.length > 0 && <i>{clues.map((food) => FOOD_ICONS[food]).join(" ")}</i>}
          </button>;
        })}
      </div>
